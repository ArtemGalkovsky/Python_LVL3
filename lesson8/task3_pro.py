...
def get_notebook_data(notebook):
    model = notebook.select_one(".result__name").text
    price_element = notebook.select_one(".g-item-data")
    if price_element is None:
        return None

    price = float(price_element.text.replace("р", "").replace(" ", "").replace(",", "."))
    url = notebook.select_one("a").attrs.get("href")
    model_split = model.split()

    if model_split[0] == "Ноутбук":
        manufacturer = model_split[1]
    else:
        manufacturer = model_split[2]

    notebook_advanced_specs = get_notebook_specs(url)
    ...
    specs.update(notebook_advanced_specs)
    ...
    with open("parsed_notebooks.txt", "a+", encoding="UTF-8") as fl:
        ...
        while 1:
            ...
            for notebook in notebooks:
                data = get_notebook_data(notebook)
                ...
                print(f"Парсим {f'{page} стр.':<5} {model:<60} {f'{price}р':<15} {data.get('url')}")
                # print(data)
    ...
    if ask_parse and input("Хотите ли заново стырить информацию с сайта? Д/н или Y/n > ").lower() in ("y", "д"):
        ...
        if show_all_of_filter.lower() == "д":
            ...
            for notebook_data in data:
                ...
                model = model[:51] + "..." if len(model) > 50 else model
                print(f"{model:<60} {f'{price}р':<15} {notebook_data.get('url')}")
        else:
            ...
            for notebook in filtered_notebooks:
                ...
                print(f"{model:<60} {diagonal:<7} {str(notebook.get('price')) + 'р':<10} "
                      f"{str(notebook.get('summary_ram')) + ' Гб ОП.П.':<15}"
                      f" {notebook.get('ram_type'):<6} {notebook.get('storage')} Гб памяти!   {notebook.get('url')}")
